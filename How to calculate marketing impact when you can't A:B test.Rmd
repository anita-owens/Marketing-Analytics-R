...
CMO: Hey! We started running some social ads a few weeks ago. Our agency
designed some cool creatives and we're getting a lot of traffic.

Me: Cool!

CMO: We need to know if we see an impact. I have to report back to the
CEO tomorrow.
...
Something like this scenario is playing out somewhere in some company. Often businesses make changes to processes in the hope that the changes increases revenue, customer acquisition, customer retention, etc.

However, I know from experience that the best way to measure these types of changes is through experimentation and A/B testing.

Every organization should have a testing infrastructure in place to
guide decision-making & evaluate ideas. In reality, most A/B tests fail
or are not significant (bad design, trivial changes, too many changes,
etc.)
...
pacman::p_load(pacman,
  tidyverse, openxlsx, psych, ggcorrplot, corrplot, cowplot)
```
...
The first that we want to do is define our (before and after) time periods, we need to add a column for the advertising periods.
...
p1 <- ggplot(df_final, aes(sales_date, revenue)) + geom_line() + theme_minimal() 
p2 <- ggplot(df_final, aes(revenue, advert_period)) + geom_boxplot() + theme_minimal() 
...
ads_group <- before_after_df %>% 
  ...
  arrange(desc(advert_period))
ads_group

#Difference
ads_group
```
Let's extract the average revenue for each period and calculate the percentage difference.
...
With the average difference in revenue, we can calculate the uplift. First, let's grab the average spend.
